# Create basic PNG files for MonoGame
$contentPath = "Core\Content"

# Create a simple 1x1 pixel PNG files
$bytes = @(
    0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52,
    0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x02, 0x00, 0x00, 0x00, 0x90, 0x77, 0x53,
    0xDE, 0x00, 0x00, 0x00, 0x0C, 0x49, 0x44, 0x41, 0x54, 0x08, 0x99, 0x01, 0x01, 0x00, 0x00, 0x00,
    0xFF, 0xFF, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0xE2, 0x21, 0xBC, 0x33, 0x00, 0x00, 0x00, 0x00,
    0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60, 0x82
)

# Create texture files
$textures = @(
    "splashscreen.png",
    "menu_background.png", 
    "cursor.png",
    "inventory_icon.png",
    "character.png",
    "door.png",
    "panel.png"
)

foreach ($texture in $textures) {
    $path = Join-Path $contentPath $texture
    [System.IO.File]::WriteAllBytes($path, $bytes)
    Write-Host "Created: $path"
}

Write-Host "Basic textures created successfully!"